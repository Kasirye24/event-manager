<?php

declare(strict_types=1);

use Phinx\Migration\AbstractMigration;

final class CreateSessionsTable extends AbstractMigration
{
    /**
     * Change Method.
     *
     * Write your reversible migrations using this method.
     *
     * More information on writing migrations is available here:
     * https://book.cakephp.org/phinx/0/en/migrations.html#the-change-method
     *
     * Remember to call "create()" or "update()" and NOT "save()" when working
     * with the Table class.
     */
    public function change(): void
    {
        $table = $this->table('sessions');
        $table->addColumn('event_id', 'integer', ['limit' => 11, 'signed' => false])
            ->addColumn('type', 'date', ['limit' => 255, 'null' => true])
            ->addColumn('date', 'date', ['null' => true])
            ->addColumn('start_time', 'time', ['null' => true])
            ->addColumn('end_time', 'time', ['null' => true])
            ->addColumn('created_at', 'datetime', ['default' => 'CURRENT_TIMESTAMP'])
            ->addColumn('updated', 'timestamp', ['null' => true])
            ->addColumn('is_deleted', 'boolean', ['default' => false])
            ->addColumn('user_id', 'integer', ['limit' => 11, 'signed' => false])
            ->addForeignKey('event_id', 'events', 'id', ['constraint' => 'event_sessions_fk'])
            ->addForeignKey('user_id', 'users', 'id', ['constraint' => 'user_sessions_fk'])
            ->create();
    }
}
